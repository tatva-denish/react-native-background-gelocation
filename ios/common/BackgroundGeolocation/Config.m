...
@synthesize driverid, driver_availibility_status, access_token, stationaryRadius, distanceFilter, desiredAccuracy, _debug, activityType, activitiesInterval, _stopOnTerminate, url, syncUrl, syncThreshold, httpHeaders, _saveBatteryOnBackground, maxLocations, _pauseLocationUpdates, locationProvider, _template;
...
+(instancetype) fromDictionary:(NSDictionary*)config
{
    ...
    if (isNotNull(config[@"driverid"])) {
        instance.driverid = config[@"driverid"];
    }
    if (isNotNull(config[@"driver_availibility_status"])) {
        instance.driver_availibility_status = config[@"driver_availibility_status"];
    }
//    if (isNotNull(config[@"access_token"])) {
//        instance.access_token = config[@"access_token"];
//    }
...
+ (instancetype) merge:(Config*)config withConfig:(Config*)newConfig
{
    ...
    if ([newConfig hasDriverid]) {
        merger.driverid = newConfig.driverid;
    }
    if ([newConfig hasDriver_Availibility_Status]) {
        merger.driver_availibility_status = newConfig.driver_availibility_status;
    }
    if ([newConfig hasAccess_Token]) {
        merger.access_token = newConfig.access_token;
    }
...
-(id) copyWithZone: (NSZone *) zone
{
    ...
    if (copy) {
        ...
        copy.driver_availibility_status = driver_availibility_status;
        copy.access_token = access_token;
...
- (BOOL) hasDriver_Availibility_Status {
    return driver_availibility_status.boolValue;
}

//- (BOOL) hasAccess_Token
//{
//    return access_token != nil;
//}
- (NSString*) hasAccess_Token
{
    if (access_token == nil) {
        access_token = @"";
    }
...
- (NSDictionary*) toDictionary
{
    ...
    if ([self hasDriverid]) [dict setObject:self.driverid forKey:@"driverid"];
    if ([self hasDriver_Availibility_Status]) [dict setObject:self.driver_availibility_status forKey:@"driver_availibility_status"];
    if ([self hasAccess_Token]) [dict setObject:self.access_token forKey:@"access_token"];
...
{
    return [NSString stringWithFormat:@"Config: distanceFilter=%@ driverid=%@ driver_availibility_status=%@  stationaryRadius=%@ desiredAccuracy=%@ activityType=%@ activitiesInterval=%@ isDebugging=%@ stopOnTerminate=%@ url=%@ syncThreshold=%@ maxLocations=%@ httpHeaders=%@ pauseLocationUpdates=%@ saveBatteryOnBackground=%@ locationProvider=%@ postTemplate=%@", self.distanceFilter, self.driverid, self.driver_availibility_status, self.stationaryRadius, self.desiredAccuracy, self.activityType, self.activitiesInterval, self._debug, self._stopOnTerminate, self.url, self.syncThreshold, self.maxLocations, self.httpHeaders, self._pauseLocationUpdates, self._saveBatteryOnBackground, self.locationProvider, self._template];
